...
const validateToken = require('../helper'); //Crear budget
...
router.post("/budget", validateToken, express.json(), async function (req, res) {
  ...
  await budget.save().then(data => res.json(data)).catch(error => res.json({
    message: error
  }));
}); //obtener budget
...
router.get("/budget/:id", validateToken, async function (req, res) {
  ...
  await budgetSchema.findById(id).then(data => res.json(data)).catch(error => res.json({
    message: error
  }));
});
/**
...
router.get("/budget", validateToken, async function (req, res) {
  ...
  res.header('Access-Control-Allow-Methods', 'GET,PUT,POST,DELETE,OPTIONS');
  res.header("Access-Control-Allow-Headers", 'Origin,X-Requested-With,Content-Type,Accept,content-type,application/json');
  await budgetSchema.find().then(data => res.json(data)).catch(error => res.json({
    message: error
  }));
});
/**
...
router.get("/budgetByExpense/:id", validateToken, async function (req, res) {
  ...
  let totalBudgetList = [];
  const expenseList = await expenseSchema.find({
    idOperation: id
  }).lean();
  const budgetList = await budgetSchema.find().lean();
  if (budgetList.length > 0) budgetList.map(x => {
    let total = 0;
    ...
    if (typeof filterExpenseList !== 'undefined') {
      ...
      totalBudgetList.push({
        name: x.idCategory.split("|")[0],
        budgetAmount: x.amount,
        type: x.type,
        amount: total,
        exceeded: x.amount < total ? true : false
      });
    }
  });
