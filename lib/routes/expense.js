...
router.post("/expense", validateToken, express.json(), function (req, res) {
  ...
  expense.save().then(data => res.json(data)).catch(error => res.json({
    message: error
  }));
});
/**
...
router.get("/expense/:id", validateToken, function (req, res) {
  ...
  expenseSchema.findById(id).then(data => res.json(data)).catch(error => res.json({
    message: error
  }));
}); //listar expense
...
router.get("/expense", validateToken, function (req, res) {
  ...
  res.header('Access-Control-Allow-Methods', 'GET,PUT,POST,DELETE,OPTIONS');
  res.header("Access-Control-Allow-Headers", 'Origin,X-Requested-With,Content-Type,Accept,content-type,application/json');
  expenseSchema.find().then(data => res.json(data)).catch(error => res.json({
    message: error
  }));
}); //actualizar expense
