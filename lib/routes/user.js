...
router.post("/user", express.json(), async function (req, res) {
  ...
  await user.save().then(data => res.json(data)).catch(error => res.json({
    message: error
  }));
}); //obtener user
...
router.get("/user/:id", validateToken, async function (req, res) {
  ...
  await userSchema.findById(id).then(data => res.json(data)).catch(error => res.json({
    message: error
  }));
});
/**
...
router.get("/user", validateToken, async function (req, res) {
  ...
  res.header('Access-Control-Allow-Methods', 'GET,PUT,POST,DELETE,OPTIONS');
  res.header("Access-Control-Allow-Headers", 'Origin,X-Requested-With,Content-Type,Accept,content-type,application/json');
  await userSchema.find().then(data => res.json(data)).catch(error => res.json({
    message: error
  }));
}); //update user
